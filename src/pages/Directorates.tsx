...
const Directorates: React.FC = () => {
  ...
  const currentDirectorate = isDirectorate ? user?.username : null;
  
  // Filter directorates based on search term
  const filteredDirectorates = directorates.filter(directorate => 
    directorate.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold tracking-tight">Müdürlükler</h1>
      </div>
      ...
      <Tabs defaultValue="directorates">
        <TabsList>
